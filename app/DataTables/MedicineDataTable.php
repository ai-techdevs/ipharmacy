...
class MedicineDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'admin.medicines.action')
            ->editColumn('created_at', fn($query)=> \Carbon\Carbon::parse($query->created_at)->format('jS M, Y') ?? "")
            ->editColumn('uses', function ($query) {
                return \Str::limit(strip_tags($query->uses), 50);
            })
    ...
    {
        return $model->newQuery()->select(["medicines.*"])->orderBy('updated_at', 'desc');
    }
